...
function processRecords (records, isGzipCompressedRecordFound) {
    return records.map(r => {
        const data = loadJsonGzipBase64(r.data, isGzipCompressedRecordFound)
...
function loadJsonGzipBase64 (base64Data, isGzipCompressedRecordFound) {
    const base64DecodedData = Buffer.from(base64Data, 'base64')
    let payload
    try {
        payload = JSON.parse(zlib.gunzipSync(base64DecodedData))
        if (isGzipCompressedRecordFound != null) {
            isGzipCompressedRecordFound.value = true
        }
    } catch (e) {
        payload = JSON.parse(base64DecodedData)
    }
    return payload
}

async function publishCloudWatchMetric (isGzipCompressed, region, lambdaFunctionName) {
    const cloudwatch = new CloudWatchClient({ region: region, maxAttempts: 6 })
    const params = {
        MetricData: [
            {
                MetricName: 'CloudWatchProcessorLambda_IncomingCompressedData',
                Dimensions: [
                    {
                ...
                Unit: 'Count',
                Value: isGzipCompressed ? 1 : 0
            }
        ],
        Namespace: 'FirehoseCloudWatchLogProcessorLambda'
    }
...
export const handler = async (event) => {
    ...
    const isGzipCompressedRecordFound = { value: false }
    const records = processRecords(event.records, isGzipCompressedRecordFound)
    try {
        await publishCloudWatchMetric(isGzipCompressedRecordFound.value, region, process.env.AWS_LAMBDA_FUNCTION_NAME)
    } catch (error) {
        console.error('Exception occurred while publishing metrics to cloudwatch. Please make sure that the lambda role has permission to emit metrics to cloudwatch. :: ' + error.message)
    }
    let projectedSize = 0
    ...
    records.forEach((rec, idx) => {
        ...
        if (rec.data.length > 6000000) {
            const cwlRecord = loadJsonGzipBase64(originalRecord.data, null)

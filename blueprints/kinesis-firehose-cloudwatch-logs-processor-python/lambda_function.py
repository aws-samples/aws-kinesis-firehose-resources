...
def processRecords(records, lambdaFunctionName):
    isGzipCompressedRecordFound = [False]
    for r in records:
        data = loadJsonGzipBase64(r['data'], isGzipCompressedRecordFound)
        recId = r['recordId']
    ...
    try:
        publishCloudWatchMetric(isGzipCompressedRecordFound[0], lambdaFunctionName)
    except:
        print("Exception occurred while publishing metrics to cloudwatch. Please make sure that the lambda role has permission to emit metrics to cloudwatch.")
...
def isPayloadGzipCompressed(data):
    if len(data) >= 2:
        return data[0] == 0x1f and data[1] == 0x8b
    return False

def publishCloudWatchMetric(isGzipCompressedData, lambdaFunctionName):
    lambdaDimension = [
        {
        ...
        MetricData=[
            {
                ...
                'Dimensions': lambdaDimension,
                'Value': 1 if isGzipCompressedData else 0,
                'Unit': 'Count'
            },
        ]
    )

def loadJsonGzipBase64(payload, isGzipCompressedRecordFound):
    base64DecodedData = base64.b64decode(payload)
    try:
        cwlRecord = json.loads(gzip.decompress(base64DecodedData))
        if isGzipCompressedRecordFound is not None:
            isGzipCompressedRecordFound[0] = True
    except:
        cwlRecord = json.loads(base64DecodedData)
...
def lambda_handler(event, context):
    ...
    records = list(processRecords(event['records'], context.function_name))
    projectedSize = 0
    ...
    for idx, rec in enumerate(records):
        ...
        if len(rec['data']) > 6000000:
            cwlRecord = loadJsonGzipBase64(originalRecord['data'], None)
